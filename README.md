...
- render: a python web service (fastapi/uvicorn with a health check) plus a static site for the frontend build
- docker compose: a full self-hosted stack with postgres, redis, neo4j, backend, and frontend behind nginx, with a separate production overlay
- continuous integration: the backend test suite and a frontend production build run automatically on every change
