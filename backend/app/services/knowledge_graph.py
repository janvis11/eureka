...
class KeywordExtractor:
    ...
    def extract_keyphrases(self, text: str, num_phrases: int = 10) -> List[str]:
        ...
        If a local HF generator is available and enabled in settings, use it
        to extract high-quality keyphrases. Otherwise, fall back to a
        simple capitalized-sequence heuristic.
        """
        # Prefer HF-based extraction if available
        if self.hf_client and self.settings.HF_USE_LOCAL_GENERATOR:
            try:
                prompt = f"Extract the top {num_phrases} keyphrases from the text below. Return a comma-separated list with no explanation.\n\nText:\n" + text[:2000]
                resp = self.hf_client.generate(prompt, max_length=128)
                # Expect comma-separated phrases
                if resp:
